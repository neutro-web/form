...
**Scope note — this entire audit measures the FULL tier only, never the minimal tier.** Every bench block in `bench/suites/core/` runs through `bench/adapters/neutro.ts`, which imports `createForm` from `@neutro/form-core` (`packages/core/src/index.ts`) — the full tier. There is no `@neutro/form-core/minimal` benchmark suite. This matters because:
- **Runtime fixes (Task 3, Task 5) apply to `setFieldValue`/`set`/`get`, which live entirely inside `engine.ts`'s `createCoreForm` — the exact function `minimal.ts` calls and returns unwrapped, with no extra layer on top.** By construction, minimal should inherit both the original regression and this audit's fix identically. This is a structural inference from reading the source, **not an independently measured fact** — no bench has ever run against `@neutro/form-core/minimal` to confirm it. If a future performance-sensitive change to `minimal.ts` or `createCoreForm` is made, this gap (no minimal-tier bench suite) should be closed rather than re-relying on the same inference.
- **The bundle-size fix (Task 4) is full-tier-only by construction, not by measurement gap.** `features/array-ops.ts` (where Task 4's consolidation lives) is one of the four clusters `minimal.ts` never imports at all (per release-gate item 2's design — array-ops/dom-bridge/persistence/computed-fields are all excluded from minimal). There is no code path for that fix to affect minimal's bundle. Measured minimal-tier bundle size across this audit: 8,128 → 8,127 gzip bytes (-1 byte) — consistent with measurement noise from re-running the same esbuild+gzip script, not a real functional change; there is no plausible mechanism in the diff for a genuine -1-byte effect on a bundle that never contained the changed code.
