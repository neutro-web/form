...
**Visual devtools panel** — `createNeutroFormDevtoolsPanel()`, an in-page UI overlay/panel variant of `devtools()` beyond the console-only devtools from 0.1.0. See [API Reference → `createNeutroFormDevtoolsPanel`](/api/devtools#createneutroformdevtoolspanel-form-options).
